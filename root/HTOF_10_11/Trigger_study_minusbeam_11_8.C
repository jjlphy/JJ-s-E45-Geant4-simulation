// -*- C++ -*-
// Trigger_study_minusbeam_10_28_HTOFselect.C  (2025-10-29 → 2025-11-07 updated)
// - Minus-beam, OR-based BeamVeto over adjacent pairs
// - Baseline(Section0), Sections1–6
// - NEW: Survived combo listing + ROOT-CAUSE diagnostics (EM/Secondary/Primary-π− only)
//   * Per-tile composition flags (hasEM, hasSecondary, allPiPrimary)
//   * Per-combo category counters + Top-N combos by count
// - NEW (2025-11-07): Trig3/Trig4 (MP>=1) 추가, 그리고 모든 경우에 대해
//                     "HTOF 1–4 제외 안 함(no-excl)" 병렬 집계 추가
//
// Usage:
//   root -l
...
static const int PDG_PI_MINUS= -211;
...
// ===== OR-logic BeamVeto over adjacent pairs (19–24 중심) =====
...
struct Counts {
  ...
  // [신규] Trig1/2 의 "no-excl (MP 제외 없음)" 버전
  long long N_trig1_noexcl=0;
  long long N_trig2_tight_noexcl=0, N_trig2_fit_noexcl=0;
  long long N_trig2_wide_noexcl=0,  N_trig2_ultra_noexcl=0;

  // [신규] Trig3/Trig4 (MP≥1, MP-excl)
  long long N_trig3=0;
  long long N_trig4_tight=0, N_trig4_fit=0, N_trig4_wide=0, N_trig4_ultra=0;

  // [신규] Trig3/Trig4 (MP≥1, no-excl)
  long long N_trig3_noexcl=0;
  long long N_trig4_tight_noexcl=0, N_trig4_fit_noexcl=0;
  long long N_trig4_wide_noexcl=0,  N_trig4_ultra_noexcl=0;

  // diagnostics (full-valid) presence
  long long have19=0, have20=0, have21=0, have22=0, have23=0, have24=0;

  // Section0-only
  long long N_trig1_with_1_4=0, N_trig1_with_0_5=0;

  // survived combo tallies (Trig2: excl 경로 기준)
  std::map<std::string,long long> Comb_tight, Comb_fit, Comb_wide, Comb_ultra;

  // root-cause tallies (by veto region; Trig2: excl 경로 기준)
  struct RC {
    long long ALL_PI_PRIMARY=0;   // 모든 유효 타일이 Primary π− 만
    long long HAS_EM=0;           // e±/γ 기여 포함
    long long HAS_SECONDARY=0;    // 비-Primary 기여 포함
    // energy footprint
    double minSumE = 1e99;        // 생존 이벤트들의 타일 ΣEdep 중 최소값
    double maxSumE = 0.0;
    // Top-N combos per category
...
static Counts ProcessRange_HTOFselect(TTree* T,
                           ...
                           const std::set<int>& excludePDG) // 완전 PDG 제외 (디버그)
{
  ...
  for(Long64_t ie=0; ie<N; ++ie){
    ...
    std::map<int,TileInfo> info_full; // per tile → ΣE, flags (fullExclude만 반영)
    std::map<int,double>   accum_mp;  // MP-only exclusion 적용 버전
    std::map<int,double>   accum_all; // no-exclusion (MP에서 아무 타일도 제외 안 함)
    ...
    if(HTOF){
      for(size_t i=0;i<HTOF->size();++i){
        ...
        // --- full-valid용 타일 구성 정보 누적 (fullExclude만 적용) ---
        if(!fullExclude.count(tid)){
          ...
          const int org = p.GetUniqueID(); // OriginTag integer
          if(org!=primaryOriginCode) tf.hasSecondary = true;

          const bool isPrimaryPi = (org==primaryOriginCode && pdg==PDG_PI_MINUS);
        ...
        // --- mp-valid (no-exclusion) ---
        accum_all[tid] += ed;
      }
    }

    // 타일 유효 판정
    std::set<int> tiles_full, tiles_mp, tiles_mp_noexcl;
    ...
    for(const auto& kv:accum_all) if(kv.second      >=thrHTOF) tiles_mp_noexcl.insert(kv.first);
    ...
    if(beam){
      ...
      // (B) Trig1/Trig2 (MP≥2, no-excl)
      if( (int)tiles_mp_noexcl.size()>=2 ){
        C.N_trig1_noexcl++;
        const auto v = EvalVeto_ORpairs(tiles_full);
        if(!v.tight) C.N_trig2_tight_noexcl++;
        if(!v.fit)   C.N_trig2_fit_noexcl++;
        if(!v.wide)  C.N_trig2_wide_noexcl++;
        if(!v.ultra) C.N_trig2_ultra_noexcl++;
      }

      // (C) [신규] Trig3/Trig4 (MP≥1, MP-excl)
      if( (int)tiles_mp.size()>=1 ){
        C.N_trig3++;
        const auto v = EvalVeto_ORpairs(tiles_full);
        if(!v.tight) C.N_trig4_tight++;
        if(!v.fit)   C.N_trig4_fit++;
        if(!v.wide)  C.N_trig4_wide++;
        if(!v.ultra) C.N_trig4_ultra++;
      }

      // (D) [신규] Trig3/Trig4 (MP≥1, no-excl)
      if( (int)tiles_mp_noexcl.size()>=1 ){
        C.N_trig3_noexcl++;
        const auto v = EvalVeto_ORpairs(tiles_full);
        if(!v.tight) C.N_trig4_tight_noexcl++;
        if(!v.fit)   C.N_trig4_fit_noexcl++;
        if(!v.wide)  C.N_trig4_wide_noexcl++;
        if(!v.ultra) C.N_trig4_ultra_noexcl++;
      }

      // Section0용 보조 카운터 (기존 유지)
                         ...
                         const std::set<int>& mpOnlyExclude, int TOPN)
{
  ...
  std::cout<<"  Trig1 (Beam && MP>=2, excl)      : "<<C.N_trig1<<"  ("<<p(C.N_trig1)<<" %)\n";
  std::cout<<"  Trig1 (Beam && MP>=2, no-excl)   : "<<C.N_trig1_noexcl<<"  ("<<p(C.N_trig1_noexcl)<<" %)\n";
  std::cout<<"  Trig3 (Beam && MP>=1, excl)      : "<<C.N_trig3<<"  ("<<p(C.N_trig3)<<" %)\n";
  std::cout<<"  Trig3 (Beam && MP>=1, no-excl)   : "<<C.N_trig3_noexcl<<"  ("<<p(C.N_trig3_noexcl)<<" %)\n";

  std::cout<<"[Trig2 survivors (!veto | from Trig1)]\n";
  std::cout<<"  tight (excl / no-excl) : "<<C.N_trig2_tight<<" / "<<C.N_trig2_tight_noexcl<<"\n";
  std::cout<<"  fit   (excl / no-excl) : "<<C.N_trig2_fit  <<" / "<<C.N_trig2_fit_noexcl  <<"\n";
  std::cout<<"  wide  (excl / no-excl) : "<<C.N_trig2_wide <<" / "<<C.N_trig2_wide_noexcl <<"\n";
  std::cout<<"  ultra (excl / no-excl) : "<<C.N_trig2_ultra<<" / "<<C.N_trig2_ultra_noexcl<<"\n";

  std::cout<<"[Trig4 survivors (!veto | from Trig3)]\n";
  std::cout<<"  tight (excl / no-excl) : "<<C.N_trig4_tight<<" / "<<C.N_trig4_tight_noexcl<<"\n";
  std::cout<<"  fit   (excl / no-excl) : "<<C.N_trig4_fit  <<" / "<<C.N_trig4_fit_noexcl  <<"\n";
  std::cout<<"  wide  (excl / no-excl) : "<<C.N_trig4_wide <<" / "<<C.N_trig4_wide_noexcl <<"\n";
  std::cout<<"  ultra (excl / no-excl) : "<<C.N_trig4_ultra<<" / "<<C.N_trig4_ultra_noexcl<<"\n";

  std::cout<<"[Beam Veto efficiency (%), = 100 - Trig2_excl/Beam*100]\n";
  ...
  std::cout<<"[Beam-induced background (counts) @ Beam=1,000,000, from Trig2_excl]\n";
  ...
  std::cout<<"[Veto fired counts (from Trig1_excl)]\n";
  ...
  // ---- Survived combo lists (Top-N) ---- (Trig2_excl 기준)
  ...
  // ---- Root-cause summary (Trig2_excl 기준) ----
...
void Trigger_study_minusbeam_11_8(const char* filename,
                                   ...
                                   int primaryOriginCode=0,          // TrackTag::EOrigin::kPrimaryBeam (기본 0 가정)
                                   const char* excludePDGCSV="" ,    // "11,-11,22" 등 (완전 제외 디버그)
                                   int TOPN=20)
{
  ...
  auto C1 = ProcessRange_HTOFselect(T, S1_lo, S1_hi, thrBH2, thrHTOF,
                                    hasBH2edep, hasHTOFedep, fullExclude, MP_EXCL_1_3,
                                    primaryOriginCode, excludePDG);
  ...
  auto C4 = ProcessRange_HTOFselect(T, S1_lo, S1_hi, thrBH2, thrHTOF,
                                    hasBH2edep, hasHTOFedep, fullExclude, MP_EXCL_4_6,
                                    primaryOriginCode, excludePDG);
  ...
  // ---- Print ----
  PrintSection("Section 1 (Full Beam) MP-excl{1,2,3,4}", C1, S1_lo, S1_hi, MP_EXCL_1_3, TOPN);
  PrintSection("Section 2 (Narrow 1) MP-excl{1,2,3,4}",  C2, S2_lo, S2_hi, MP_EXCL_1_3, TOPN);
  PrintSection("Section 3 (Narrow 2) MP-excl{1,2,3,4}",  C3, S3_lo, S3_hi, MP_EXCL_1_3, TOPN);

  PrintSection("Section 4 (Full Beam) MP-excl{0,1,2,3,4,5}", C4, S1_lo, S1_hi, MP_EXCL_4_6, TOPN);
  PrintSection("Section 5 (Narrow 1) MP-excl{0,1,2,3,4,5}",  C5, S2_lo, S2_hi, MP_EXCL_4_6, TOPN);
  PrintSection("Section 6 (Narrow 2) MP-excl{0,1,2,3,4,5}",  C6, S3_lo, S3_hi, MP_EXCL_4_6, TOPN);
}

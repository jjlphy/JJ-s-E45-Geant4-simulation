// BVH_Overkill_ver2.C
// (Use overlay triplets {BH2, BVH_U, BVH_D} from BVH_3D_ver2 to compute Overkill rate on 2pi sample)
//
// 사용법 예:
//   root -l
//   .L BVH_Overkill_ver2.C+
//   run_overkill();
//
// 입력 파일 형식 (혼합 허용):
//   {3, 4, 17}
...
//   - 분자(numerator): 분모 조건을 만족하며, Overlay Triplets 중 하나와
//                     (BH2, BVH_U, BVH_D) 조합이 일치 → Veto에 의해 잘림
//   - Overkill rate = numerator / denominator
//
// 주의:
//   - EX(제외) 트리플릿은 입력 리스트에 이미 빠졌다고 가정(시각화와 동일).
//   - 세그먼트 개수(Ver2): BVH_U=22, BVH_D=32
...
// ===== Helper: (BH2,BVHU,BVHD) → 단일 int 키 =====
static inline int key3(int h,int u,int d){ return (h<<16) | (u<<8) | d; }

// ===== Parser: overlay_triplets_ver2.txt 읽어서 (h,u,d) 집합 구성 =====
...
// ================== Main Overkill Computation (Ver2) ==================
...
void run_overkill() {
  ...
  const char* overlay_txt = "overlay_triplets_ver2.txt"; // BVH_3D_ver2 출력
